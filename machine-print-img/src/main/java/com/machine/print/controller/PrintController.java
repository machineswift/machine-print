...
import com.machine.print.threadpool.PrintImgTask;
...
public class PrintController {
	...
	public String removeQueueCode(@RequestBody String jsonStr) {
		...
		if (PrintCacheUtils.concurrentMap.containsKey(uuid)) {
			logger.info("The pdf2img task is executing , can not remove ! uuid : {}", uuid);
	...
	public String clearQueue(@RequestBody String printerName) {
		...
		logger.info("=========================== {}" ,PrintCacheUtils.concurrentMap );
	...
	public String elePrintMessage_BK(@RequestBody String elePrintMessage) {
		...
		try {
			...
			/* 通过单例线程池保证打印顺序 */
			PrintThreadPool.getInstance().getPtintExecutor(printerName).execute(new PrintImgTask(uuid, printerName));
			...
			logger.error("PrintBusinessException CODE:{} :{}  ExceptionMessage:{}  ElePrintMessage:{}",
					e.getErrorCode(), PrintExceptions.getValueByKey(e.getErrorCode()), e.getMessage(), elePrintMessage);
			/* 清理数据--start */
			PrintCacheUtils.removeCacheByKey(printerName, uuid);
			PrintFileUtils.delTempFileByUUID(uuid);
